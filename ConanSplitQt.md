...
Qt5-debug-x86_64-data.cmake
Qt5-Target-debug.cmake
Qt5Config.cmake
Qt5Targets.cmake
Qt5CoreConfig.cmake
```

Let's see some interesting lines in `Qt5-debug-x86_64-data.cmake`:
```cmake
set(QtCore_PACKAGE_FOLDER_DEBUG "~/.conan/data/QtCore/0.0.0/user/channel/package/1a2b3c4detc")
set(QtCore_BUILD_MODULES_PATHS_DEBUG "${QtCore_PACKAGE_FOLDER_DEBUG}/conan-qt5-config.cmake")
set(QtCore_BUILD_DIRS_DEBUG "${QtCore_PACKAGE_FOLDER_DEBUG}/")
```

Then in `Qt5-Target-debug.cmake`:
```cmake
set(CMAKE_MODULE_PATH ${QtCore_BUILD_DIRS_DEBUG} ${CMAKE_MODULE_PATH})
set(CMAKE_PREFIX_PATH ${QtCore_BUILD_DIRS_DEBUG} ${CMAKE_PREFIX_PATH})
```

Then in `Qt5Targets.cmake`:
```cmake
include("${CMAKE_CURRENT_LIST_DIR}/Qt5-debug-x86_64-data.cmake")
include("${CMAKE_CURRENT_LIST_DIR}/Qt5-Target-debug.cmake")
```

Finally in `Qt5Config.cmake`:
```cmake
include("${CMAKE_CURRENT_LIST_DIR}/Qt5Targets.cmake")

foreach(_BUILD_MODULE ${QtCore_BUILD_MODULES_PATHS_DEBUG} )
...
Initially, the `CMAKE_PREFIX_PATH` only contains the path to the build directory.

`find_package()` locates `Qt5Config.cmake` in the build directory.

`find_package()` also loads this `Qt5Config.cmake` with variables,
like `Qt5_FIND_COMPONENTS` defined.
